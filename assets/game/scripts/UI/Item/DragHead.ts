...
import EditHeadArea from "./EditHeadArea";


const { ccclass, property } = cc._decorator;

@ccclass
export default class DragHead extends cc.Component {
    ...
    private touchStart(event: cc.Event.EventTouch) {
        ...
        ListenerManager.dispatch(EventType.DRAG_HEAD, pos2);
    }
    ...
    private touchEnd(event: cc.Event.EventTouch) {
        ...
        this.gezi.children.forEach(gezi => {
            if (gezi.getComponent(EditHeadArea).isPosInRect(cc.v2(pos2.x, pos2.y))) {
                isInRect = true;
                gezi.getComponent(EditHeadArea).fill(this.node);
            }
        });
        if (!isInRect) {
            this.node.parent = this.rootNode;
        ...
        ListenerManager.dispatch(EventType.DRAG_HEAD_END);
    }
